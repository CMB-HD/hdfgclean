...
Below is a brief summary of the foreground cleaning procedure of MacInnis et. al. 2026. (You do not need to read this section before running the code we provide here, but it may provide some helpful context.)
...
- The `FGClean` class in the `fgclean.py` module is designed to run the FG cleaning on any set of maps compatible with the [pixell](https://pixell.readthedocs.io) python package. 
  - This can be done with the provided `run_fgclean.py` python script.
...
- A path to an `outdir_dir` where you would like to put the files saved by `HDFGClean`.
  - You may use the same `output_dir` if you run the FG cleaning on different sets of maps. However, if you want to run the full FG cleaning again on the same set of maps, you must provide a different `output_dir`. 
...
The `FGClean` class (in the `fgclean.py` module) contains the methods used to FG-clean a set of maps. As opposed to the `HDFGClean` class (designed to run FG cleaning on maps from `hdsims`), `FGClean` can take in any set of maps compatible with [pixell](https://pixell.readthedocs.io) (in particular the functions in `pixell.enmap`).


#### Initializing `FGClean`

The `FGClean` class has three **required** arguments:
1. The path to an `output_dir` where you would like to save the output files. You should generally use a new `output_dir` for each FG cleaning run. However, you may use the same `output_dir` to re-run the FG cleaning using a different set of SNR thresholds or tSZ cluster profiles. 
2. A dictionary of `imaps`, the maps being FG-cleaned; these maps should be apodized. The dictionary must have a key (`int` or `float`; we recommend the former) for each map frequency (in GHz), and the corresponding value is the map at that frequency. This can be the actual map (as a `pixell.enmap.ndmap`), or the file name of the saved map (as a `.fits` file that can be read with `pixell.enmap.read_map`); if you are initializing `FGClean` with a configuration `.yaml` file, then the `imaps` dict in that file *must* contain the map file names, not the maps themselves.
3. A dictionary of `beam_fwhms` with the beam size at each frequency (full-width at half-maximum, in arcminutes, for a Gaussian beam profile). The keys must be the same as in the `imaps` dictionary.


`FGClean` also accepts additional, optional keyword arguments. The file `fgclean_defaults.yaml` provided here lists all of these options with their default values, and provides a brief description of each; some are also described in the `run_hdfgclean.ipynb` notebook.
- **Note**: By default, the matched filters calculated with `FGClean` will use your input maps as an estimate of the noise (this is in contrast to `HDFGClean`, which will use a second set of maps for this purpose by default). You may pass other maps at each frequency that will be used to quantify the noise in the matched filter calculations. See the [examples](#examples) for an example of how to do this.


There are two ways to initialize the `FGClean` class. The standard way is to pass arguments directly to the initialization method of the `FGClean` class, e.g.,

```python
from fgclean import fgclean
output_dir = '/path/to/output'
imaps = {90: '/path/to/90GHz_map.fits', 148: '/path/to/148GHz_map.fits'}
beam_fwhms = {90: beam_fwhm_at_90GHz, 148: beam_fwhm_at_148GHz}
fgcleanlib = fgclean.FGClean(output_dir, imaps, beam_fwhms)
```

The second way is by saving the arguments in a configuration `.yaml` file (refer to `fgclean_defaults.yaml` for an example) by calling the `FGClean.save_config` class method. You can then use this file to initialize `FGClean` by calling the the `FGClean.from_config` class method, e.g.
...
imaps = {90: '/path/to/90GHz_map.fits', 148: '/path/to/148GHz_map.fits'}
beam_fwhms = {90: beam_fwhm_at_90GHz, 148: beam_fwhm_at_148GHz}
config_file = '/path/to/my_fgclean_config.yaml'
fgclean.FGClean.save_config(config_file, imaps, beam_fwhms, output_dir=output_dir)
fgcleanlib = fgclean.FGClean.from_config(config_file)
```
...
The foreground cleaning can be run using the `run_fgclean` method of the `FGClean` class: e.g., once you have initialized the class as demonstrated above, you would just call `fgcleanlib.run_fgclean()` to run the FG cleaning on your maps. By default, after the maps have been FG-cleaned, the `run_fgclean` method will also save a mask for each map frequency to mask out any remaining |SNR| $\geq$ 5 point sources or clusters; this SNR threshold, and the apodization width used to apodize the mask holes, can be changed by passing the appropriate keyword arguments to the `run_fgclean` method.

We provide a python script, `run_fgclean.py`, which you can use to run the FG cleaning. You must pass the path to your `config_file`, which will be used to initialize `FGClean`. The following command can be used to run the FG cleaning on your maps and save the masks:

```
mpirun -np <N> python run_fgclean.py /path/to/my_fgclean_config.yaml
```

where you would replace `<N>` with the number of MPI processes to use (see the "[Using MPI](#using-mpi-strongly-recommended)" section above). Pass `--savemaps` to also save the FG-cleaned maps in addition to the catalogs of detected point sources and clusters.

There is also a `--test` option that can be passed to `run_fgclean.py`, e.g. `python run_fgclean.py /path/to/my_fgclean_config.yaml --test`. In this case, `FGClean` will be initialized using your configuration file, and a message will be printed out telling you that the initialization was successful. A second message will be printed out telling you how many smaller patches your maps will be divided into, so you know how many MPI processes to use. You may also run the command `python run_fgclean.py --help` for additional information.


**After running FG cleaning**:

In the table below, we list some of the methods of the `FGClean` class (not including the main `run_fgclean` method) that can be used to load in saved FG cleaning results, or calculate them if they haven't been saved. You should refer to the full documentation of each method (the docstring under the method definition) before using it.

- The "method name" column gives the name of a method of the `FGClean` class; e.g., the method named `run_fgclean` is called with `fgcleanlib.run_fgclean()`, where`fgcleanlib` is an instance of `FGClean`.
- We provide a brief description of each method and list any required arguments, but do not include optional keyword arguments. The "required arguments" column will be left blank if there are no required arguments.

Many of the methods will try to load some saved result (e.g. the FG-cleaned maps), and if the relevant file is not found, the result will be calculated; this could include running the full FG cleaning procedure, so you should always run all FG cleaning steps on the maps before trying to access the results.


| method name | description | required arguments |
|-------------|-------------|--------------------|
| `get_fgcleaned_maps` | dictionary of the maps after FG cleaning at each frequency |  |
| `get_map_of_subtracted_sources` | map of detected point sources | map frequency |
| `get_maps_of_subtracted_sources` | dictionary of maps of detected point sources at each frequency |  |
| `get_map_of_subtracted_clusters` | map of detected tSZ clusters | map frequency |
| `get_maps_of_subtracted_clusters` | dictionary of maps of detected tSZ clusters at each frequency |  |
| `get_masks` | dictionary of masks applied to maps at each frequency to mask out residual FGs after FG cleaning | |
| `get_catalog_of_subtracted_sources` | catalog of detected point sources | map frequency |
| `get_catalog_of_subtracted_clusters` | catalog of detected tSZ clusters |  |
| `get_spectral_index` | measured average CIB or radio spectral index between the highest or lowest frequency, respectively, to the given frequency | map frequency  and point source component (`'cib'` or `'radio'`) |
...
We provide an example, with complete instructions, to run all FG cleaning steps on a smaller (four-square-degree) set of `hdsims` maps in the `examples/hdfgclean_example_2x2.ipynb` notebook. The notebook also includes a few shorter examples that use the methods of `HDFGClean` to load in the results after the full FG-cleaning procedure has been run. We also provide a second example in the `examples/fgclean_example_2x2.ipynb` notebook that will illustrate how to use the more general `FGClean` class.
